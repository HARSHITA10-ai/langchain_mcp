...
def render_chart(
    ...
    try:
        ...
        # --- START OF ADDED CODE ---
        # This block wraps the x-axis labels to prevent overlap
        if x_col and chart_type not in ["horizontal_bar", "pie", "pairplot"]:
            try:
                # Get current labels
                ...
                # Log if wrapping fails, but don't crash the chart
                logger.warning(f"Could not wrap x-axis labels: {e}")
        # --- END OF ADDED CODE ---
# Finalize and save non-pairplot charts
        ax.set_title(title)
        if x_col: ax.set_xlabel(x_col.replace("_", " ").title())
